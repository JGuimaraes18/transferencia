...
consulta_array="SELECT id FROM core_passagem WHERE inicio LIKE '%$data_atual%';"a#!/bin/bash 
path_exe='/home/transfoper/bin/'
cmd_sql="/usr/bin/sqlite3"
bd="/home/transfoper/transferencia/database/db.sqlite3"
data_atual=`date +%Y-%m-%d`
...
consulta="select * from core_antena where id = '$antena_pesquisa';"   pesquisa=`$cmd_sql $bd "$consulta" ""`
